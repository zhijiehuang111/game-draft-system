...
export interface DraftEngine {
  ...
  handleRoomJoin(userId: string, roomId: string): { ok: true; snapshot: RoomState } | { ok: false; code: string };
...
export function createDraftEngine(deps: DraftEngineDeps): DraftEngine {
  ...
  function handleRoomJoin(userId: string, roomId: string) {
    ...
    return { ok: true as const, snapshot: room.snapshot() };
  }
  ...
  return { createRoom, handleRoomJoin, handlePickInitial, handlePickBench };
}
