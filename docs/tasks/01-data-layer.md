...
- [x] 撰寫 migration `001_init.sql`，包含 `users`、`draft_results` 2 張表
...
- [x] `packages/shared/src/types/db.ts`：與 DB 欄位對齊的 `User`、`DraftResult` 型別
...
- [x] 寫 1 個煙霧測試：能 insert user → insert results（`apps/server/src/db/smoke.ts`，`pnpm --filter @app/server smoke`，包 transaction + ROLLBACK 不留資料）
