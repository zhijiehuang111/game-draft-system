...
- [ ] 後端建立 `pg` Pool（`packages/server/src/db/pool.ts`），從 `DATABASE_URL` 載入

### Migration 機制
- [ ] 選定 migration 工具（推薦：`node-pg-migrate` 或手寫 SQL runner），加入 `package.json` scripts
- [ ] 建立 `packages/server/migrations/` 目錄
...
- [ ] 兩邊 import 驗證：`packages/server` 與 `packages/client` 均可 import shared 型別

### Repository 層
- [ ] `packages/server/src/db/repositories/users.repo.ts`：`createUser`、`findByUsername`、`findById`
- [ ] `packages/server/src/db/repositories/rooms.repo.ts`：`createRoom`、`completeRoom`、`abortRoom`、`insertPlayers`
- [ ] `packages/server/src/db/repositories/results.repo.ts`：`insertResults`（批次寫 5 筆）
